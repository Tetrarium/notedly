...
module.exports = {
	...
	deleteNote: async (parent, { id }, { models, user }) => {
		...
		// выбрасываем запрет на действия
	...
	updateNote: async (parent, { content, id }, { models, user }) => {
		// Если не полльзователь, выбрасываем ошибку авторизации
		if (!user) {
		...
		// Находим заметку
		const note = await models.Note.findById(id);
		// Если владелец записи и текущий пользователь не совпадают,
	...
	toggleFavorite: async (parent, { id }, { models, user }) => {
		// Если контент пользователя не передан, выбрасываем ошибку
		if (!user) {
		...
		// Если пользователь есть в списке, удаляем его оттуда
		// и уменьшаем значение favoriteCount на 1
		if (hasUser >= 0) {
